...
def IoU(y_true, y_pred):
    """
    Returns IoU (without thresholding, good for training loss).
    Note that this function returns 1 - IoU
    """
    SMOOTH=10.
    ...
    return 1-tf.reduce_mean((true_positive+SMOOTH)/(true_positive+false_negative+false_positive+SMOOTH))
...
def accuracy(y_true, y_pred):
    predicted = tf.where(y_pred > 0.5, 1., 0.)
    correct_count = tf.reduce_sum(1-tf.abs(y_true-y_pred), axis=[1,2])
